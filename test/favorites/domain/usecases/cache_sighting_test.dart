...
void main() {
  ...
  setUp(() {
    ...
    usecase = CacheSighting(repository: mockAppRepository);
  });
  ...
  test(
    ...
    () async {
      // arrange
      when(
        () => mockAppRepository.cacheSighting(any()),
      ).thenAnswer(
      ...
      final result = await usecase(
        CacheSightingParams(sighting: tSighting),
      );

      // assert
      expect(result, Right(tSightingsList));
      verify(() => mockAppRepository.cacheSighting(tSighting));
      verifyNoMoreInteractions(mockAppRepository);
    },
  );

  test(
    'should get [CachingException] when caching fails',
    () async {
